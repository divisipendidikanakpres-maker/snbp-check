...
"""
BAN-PDM Scraper - seluruh wilayah + akreditasi terbaru + filter status Negeri/Swasta
Output: 1 sekolah = 1 baris.
"""
...
def fetch_latest_akreditasi(npsn):
    ...
    try:
        ...
        return {"program": "", "peringkat": "", "no_sk": "", "tahun_akreditasi": "", "provinsi_riwayat": "", "tahun_berakhir": ""}
    ...
    for tr in akr_table.find_all("tr"):
        ...
        if len(cols) >= 7 and cols[0].isdigit():
            return {
                "program": cols[1], "peringkat": cols[2], "no_sk": cols[3],
                "tahun_akreditasi": cols[4], "provinsi_riwayat": cols[5], "tahun_berakhir": cols[6],
            }
...
def get_all_provinces_map():
    ...
    return {name.upper(): code for name, code in provinces}

def crawl(output_csv="sma_smk_negeri_seluruh_indonesia.csv",
          target_provinces=None,
          kabupaten_filter=None,
          status_filter="Negeri",   # "Negeri", "Swasta", atau None untuk semua
          checkpoint_every=100):
    """
    status_filter:
      "Negeri" -> hanya sekolah negeri
      "Swasta" -> hanya sekolah swasta
      None     -> semua status
    """
    ...
    print(f"Selesai. Total sekolah ({status_filter or 'SEMUA'}): {len(all_rows)} -> {output_csv}")
    return all_rows

if __name__ == "__main__":
    # Seluruh Indonesia, hanya sekolah Negeri
    # crawl(output_csv="sma_smk_negeri_seluruh_indonesia.csv", status_filter="Negeri")

    # Contoh: hanya Jawa Barat, negeri saja
    # crawl(output_csv="sma_smk_negeri_jabar.csv", target_provinces=["JAWA BARAT"], status_filter="Negeri")

    # Contoh: hanya Kota Bekasi, negeri saja
    crawl(output_csv="sma_smk_negeri_kota_bekasi.csv", target_provinces=["JAWA BARAT"], kabupaten_filter="KOTA BEKASI", status_filter="Negeri")

    # Contoh: semua status (tanpa filter)
    # crawl(output_csv="sma_smk_semua_status.csv", status_filter=None)


# note: if you want the most recent row by year instead of first displayed row, I can sort riwayat by tahun_akreditasi.